...
export default function AdminPage() {
  ...
  return (
    <div className="mx-auto w-full max-w-md px-5 py-10">
      ...
      <h1 className="display rise d2 mt-2 text-[28px]">{data.title}</h1>
      <p className="rise d2 mt-2 text-[13.5px] text-ink-3">
        {data.totalQuestions} questions ·{" "}
        {data.status === "paid" ? "unlocked ✓" : "not unlocked yet"}
      </p>

      {data.status === "preview" && (
        <div className="card-glass rise d3 mt-6 p-6 text-center">
      ...
      {shareUrl && (
        <div className="card-glass rise d3 mt-6 flex items-center justify-between gap-3 rounded-full p-2 pl-5">
          <span className="truncate text-[14px] text-ink-2">{shareUrl}</span>
          <button
          ...
          <div className="card mt-3 px-5 py-3">
            {data.leaderboard.map((p, i) => (
              <div key={`${p.name}-${i}`} className="flex items-center justify-between py-2">
                ...
                <span
                  className={`display text-[18px] ${i === 0 ? "text-rose" : "text-ink-2"}`}
                >
                  {p.score}
                </span>
              </div>
      ...
      <p className="mt-10 text-[12.5px] text-ink-4">
