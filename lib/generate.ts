 ...
 * in the browser (never the raw chat) and asks an LLM to write the quiz.
 * Provider is picked by which key is set: OPENAI_API_KEY wins, then
 * ANTHROPIC_API_KEY. Falls back to a deterministic template quiz when neither
...
async function generateWithOpenAI(prompt: string): Promise<string> {
  ...
  if (msg?.refusal) throw new Error(`OpenAI refusal: ${msg.refusal}`);
...
export async function generateQuiz(
  ...
  if (!provider) {
    console.warn(
      "[yapped] no OPENAI_API_KEY or ANTHROPIC_API_KEY set — using template quiz generator"
    );
    return { ...buildFallbackQuiz(signals, relationship), usedFallback: true };
  }

  try {
    const prompt = buildPrompt(signals, relationship);
    const text =
      provider === "openai"
